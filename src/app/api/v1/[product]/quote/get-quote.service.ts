...
export async function getQuoteForCar(data: generateQuoteDTO) {
  try {
    ...
    const payloadData = {
      ...
      quick_quote_owner_drv_exp: data.personal_info.driving_experience.toString(),
      quick_quote_owner_ncd: `${data.insurance_additional_info.no_claim_discount.toString()}%`,
      quick_quote_owner_no_of_claims: data.insurance_additional_info.no_of_claim.toString(),
    ...
    logger.info(`Response from generate quote: ${JSON.stringify(response.data)}`);
